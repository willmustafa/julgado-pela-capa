...
const scrapper = async (page, editora) => {
	...
	for await(const link of links){
		...
		try {
			let isbn = await page.$eval('.woocommerce-product-attributes-item--attribute_pa_isbn>td', el => el.innerText).catch(() => null);
			let titulo = await page.$eval('.product_title.entry-title.elementor-heading-title.elementor-size-default', el => el.innerText).catch(() => null);
			let autor = await page.$eval('.woocommerce-product-details__short-description', el => el.innerText).catch(() => null);
