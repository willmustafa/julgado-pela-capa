...
const URL = 'https://www.companhiadasletras.com.br/Busca'
const BASE_URL = 'https://www.companhiadasletras.com.br'
...
export async function scrapCompanhiaDasLetras() {
  ...
  do {
    ...
    const $ = await scraper.loadPage(URL, {
      method: 'GET',
      params: {
        action: 'buscar',
        pg: page
    ...
    isLastPage = response.data.totalPages === page

    for (const responseElement of response.data.livros) {
      let book: any = {
        title: responseElement.titulo,
        publisher: responseElement.selo,
        authors: responseElement.autores.map((el: any) => el.nome),
        group: 'companhia das letras',
        price: parseFloat(responseElement.preco?.replace('R$ ', '')?.replace(',', '.'))
      }

      try {
        console.log('entrando na pagina', BASE_URL + '/literatura/livro/' + responseElement.link)
